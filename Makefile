...
$(TWEAK_NAME)_INJECT_DYLIBS = Tweaks/Cercube/Library/MobileSubstrate/DynamicLibraries/Cercube.dylib $(THEOS_OBJ_DIR)/libFLEX.dylib $(THEOS_OBJ_DIR)/iSponsorBlock.dylib $(THEOS_OBJ_DIR)/YouPiP.dylib $(THEOS_OBJ_DIR)/YouTubeDislikesReturn.dylib $(THEOS_OBJ_DIR)/YTABConfig.dylib $(THEOS_OBJ_DIR)/YTUHD.dylib
...
CERCUBE_DEB = $(CERCUBE_PATH)/me.alfhaily.cercube_${{ env.CERCUBE_VERSION }}_iphoneos-arm.deb
CERCUBE_DYLIB = $(CERCUBE_PATH)/Library/MobileSubstrate/DynamicLibraries/Cercube.dylib
CERCUBE_BUNDLE = $(CERCUBE_PATH)/Library/Application\ Support/CercubeBundle.bundle
...
before-all::
	@if [[ ! -f $(CERCUBE_DEB) ]]; then \
		if [[ $(curl -s -w "%{http_code}" https://dl.dropboxusercontent.com/s/b01erqbp3ajc5e2/me.alfhaily.cercube_${{ env.CERCUBE_VERSION }}_iphoneos-arm.deb -o $(UYOU_DEB)) -ne 200 ]]; then \
			$(PRINT_FORMAT_ERROR) "Failed to download Cercube"; exit 1; \
		fi; \
	fi; \
	if [[ ! -f $(CERCUBE_DYLIB) || ! -d $(CERCUBE_BUNDLE) ]]; then \
		tar -xf Tweaks/Cercube/me.alfhaily.cercube_${{ env.CERCUBE_VERSION }}_iphoneos-arm.deb -C Tweaks/Cercube; tar -xf Tweaks/Cercube/data.tar* -C Tweaks/Cercube; \
		if [[ ! -f $(CERCUBE_DYLIB) || ! -d $(CERCUBE_BUNDLE) ]]; then \
